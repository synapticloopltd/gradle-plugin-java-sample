...
public class JavaSampleHelloTask extends DefaultTask {
	private Logger logger;
	private JavaSamplePluginExtension extension;

	/**
	 * Instantiate a new Java Sample Hello Task and set the group and description
	 */
	public JavaSampleHelloTask() {
		setGroup("Sample");
		setDescription("This is a sample plugin that says hello");
	...
	public void generate() {
		...
		logger.info("Invoking the hello task");
	...
	private void output() {
		if(extension.getNames().size() != 0) {
			for(String name: extension.getNames()) {
				logger.lifecycle(String.format("%s says: '%s'", name, extension.getHelloText()));
			}
		} else {
			logger.lifecycle(extension.getHelloText());
		}
	}
}
